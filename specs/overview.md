...
- the HTTP adapter exposes the same ask flow as async jobs plus status streams, optional uploaded attachments, built-in web UI GitHub SSO endpoints with signed 30-day sliding session cookies, and a signed Simple-mode API ask endpoint for integrations
