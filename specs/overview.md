...
- preserve a simple local workflow around repo scoping, sync, and Codex execution
...
- user config points to a repo catalog file under the managed repos root by default; that repo catalog stores `managedReposRoot` plus the curated repo list
- when a repo entry still uses legacy `topics` / `classifications` fields and omits `routing`, config loading drafts a fallback routing card from those legacy values so the repo catalog still provides useful hint metadata until discovery is rerun
- config can be bootstrapped from a local catalog file or discovered from a GitHub owner before being selectively added to or overridden in the repo catalog, using authenticated GitHub access from `GH_TOKEN` / `GITHUB_TOKEN` or, if those env vars are unset, the current `gh` login, including private repos visible to that credential; direct `discover-github` can omit `--owner`, prompting on a TTY and otherwise defaulting to `@accessible`; discovery uses a combined interactive selection list of new and already configured repos, defaults Enter to all new repos behind a confirmation prompt, and then refines only the selected subset with repo-content inspection, routing-card drafting from GitHub/README/route/dependency signals, a Codex cleanup pass, visible progress updates, and one final repo-catalog write
...
- asking a question uses all configured repos by default, or an explicit repo subset when provided; when multiple repos are in scope, Codex runs from the managed repos root and is told to use the repo catalog as a hint index while answering from code
- Codex-backed answers capture the final answer text plus reported token usage (`inputTokens`, `cachedInputTokens`, `outputTokens`) when the local `codex exec --json` stream includes it
...
- repo metadata is stored as a routing card with `role`, `reach`, `responsibilities`, `owns`, `exposes`, `consumes`, `workflows`, `boundaries`, `selectWhen`, and `selectWithOtherReposWhen`, so Codex has a useful hint index for owned behavior and exposed surfaces over generic ecosystem overlap; `consumes` is intentionally sparse and favors purpose-qualified labels like `product data DB` or `bulk export queue` over vendor names for generic infrastructure
