...
- config can be bootstrapped from a local catalog file or discovered from a GitHub owner before being selectively added to or overridden in local config, showing a lightweight GitHub-metadata preview first, including private repos visible to an authenticated GitHub token when available, then refining only the selected repos with repo-content inspection, size-aware topic enrichment, separate repo classifications, a Codex cleanup pass, and progress updates before writing config
