...
- config can be bootstrapped from a local catalog file or discovered from a GitHub owner before being selectively added to or overridden in local config, reusing GitHub metadata, filling blank descriptions/topics from repo-content inspection when needed, enriching topics locally with a size-aware topic budget, deriving separate repo classifications from metadata plus repo-content inspection for high-signal selection cues, running a Codex cleanup pass over the draft metadata before previewing or writing it, and emitting progress updates while discovery inspects repos
