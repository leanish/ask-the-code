...
- asking a question uses automatic repo selection by default, or an explicit repo subset when provided; automatic selection now routes on structured repo metadata, uses `selectionMode: "single"` by default to run one `gpt-5.4-mini/medium` selector pass, allows the selector to return an empty repo list when nothing looks relevant, can use `cascade` mode when confidence-threshold escalation is preferred, and can optionally keep background alternate selector model/effort comparisons for diagnostics while syncing the final repo set and running Codex for the answer
...
- repos can be pinned into automatic selection with `alwaysSelect`, and automatic selection retries malformed selector output once and then fails explicitly when the Codex selector does not return a usable repo set
- repo metadata is stored as a routing card with `role`, `reach`, `responsibilities`, `owns`, `exposes`, `consumes`, `workflows`, `boundaries`, `selectWhen`, and `selectWithOtherReposWhen`, so selection can prefer owned behavior and exposed surfaces over generic ecosystem overlap; `consumes` is intentionally sparse and favors purpose-qualified labels like `product data DB` or `bulk export queue` over vendor names for generic infrastructure
