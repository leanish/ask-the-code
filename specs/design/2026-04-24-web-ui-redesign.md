...
Replace the current hand-rolled single-string web UI with a redesigned interface that matches the provided mocks (Simple mode + Expert/admin mode), built on Hono. Keep behavior local-only during development: no PRs, no git commits initiated from this work unless the user explicitly asks later.

The redesign is visually close to the mocks but not pixel-perfect. Features present in the mocks that do not yet have backend support are rendered as inert UI stubs (Option B from brainstorming).

## Non-goals

- No new backend features (Google auth, file upload processing, history persistence, Sync Status endpoint, Add Repository flow, Discover GitHub flow). These are visual stubs only.
...
### Transitional files (removed by Step 5)

- `src/server/ui/html.ts` — deleted in Step 5.
- `src/server/api/http-server.ts` — contents migrated into `src/server/routes/*` during Step 4, file deleted in Step 4.
...
- "Sign in with Google" button (stub).
...
|---|---|
| Sign in with Google | Button renders; click shows a toast "Google sign-in isn't wired up yet." No auth flow. |
| File drop + file list | Accepts drops and click-to-browse; stores file metadata in client state; shows "Uploaded" tick visually. Files are NOT sent in `/ask`. A small banner notes "Attachments are preview-only in this build." |
...
- Keep `src/server/ui/html.ts` on disk but unreachable via HTTP (the new route shadows it).
...
- Delete `src/server/api/http-server.ts` and the `app.all("*", ...)` catch-all in `app.ts`. `createApp()` no longer takes a `legacyHandler` parameter; `main.ts` calls it with no arguments.
...
Acceptance: repo has no references to `html.ts`; docs describe the current shape; one clean commit graph ready for review. Re-run the XSS smoke check from Step 2 against the final build and record the result.
...
- Real Google auth and session management.
- Real file upload plumbing into `/ask`.
