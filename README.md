...
File attachment controls read selected files in the browser and send them with `/ask` so the model can use them during synthesis. GitHub sign-in uses local OAuth endpoints when `ATC_GITHUB_CLIENT_ID`, `ATC_GITHUB_CLIENT_SECRET`, and `ATC_AUTH_SECRET` are configured; otherwise the UI leaves the button visible but reports that SSO is not configured. Signed-in sessions use a 30-day sliding expiry. If any one of those GitHub SSO env vars is set, all three must be set or the server fails fast at startup.
